...
FilaCircular *newFilaCircular(int max) {
    ...
    fila->max = max;
    fila->n = 0;
    fila->array = (Show*)malloc(max * sizeof(Show));
    return fila;
}
...
Show desenfilerar(FilaCircular* fila) {
    ...
    Show x = fila->array[fila->primeiro];
    fila->primeiro = (fila->primeiro + 1) % fila->max;
    fila->n--;
    return x;
}

void enfilerar(FilaCircular* fila, Show elem) {
    if (fila->n == fila->max) {
        return;
    }
...
int main() {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (strcmp(comando, "I") == 0) {
            ...
            enfilerar(fila, procurar(show, 1368, id));
            soma += procurar(show, 1368, id).release_year;
            printf("[Media] %d\n", (soma / numeroDeInseridos));
