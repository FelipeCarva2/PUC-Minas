...
PilhaLinear* newPilhaLinear(int max){
    ...
    pilha->max = max;
    pilha->n = 0;
    pilha->array = (Show*)malloc(max*sizeof(Show));
    return pilha;
} 
...
void empilhar(PilhaLinear* pilha, Show elem){
...
Show desempilhar(PilhaLinear* pilha){
    if(pilha->n == 0){
        printf("Erro!");
        return show_newEmpty();
    }
    pilha->n--;
    Show x = pilha->array[pilha->n];
    return x; 
}
...
Show pegar(PilhaLinear* pilha, int pos) {
    if (pilha == NULL || pos < 0 || pos >= pilha->n) {
        fprintf(stderr, "Erro: posição inválida (%d)\n", pos);
        return show_newEmpty();
    }
    return pilha->array[pos];
}
...
int main() {
	...
	Ler();
	char id[11];
	PilhaLinear* pilha = newPilhaLinear(1000);
	...
	PilhaLinear* pilhaRemovidos = newPilhaLinear(1000);
	int n;
	scanf("%d", &n);
	for(int i = 0; i < n; i++) {
		char comando[5];
		...
		if(strcmp(comando, "I") == 0) {
            scanf(" %s", id);
            empilhar(pilha, procurar(show, 1368, id));
    ...
    for(int i = 0; i < pilhaRemovidos->n; i++){
        Show item = pegar(pilhaRemovidos, i);
        printf("(R) %s\n", item.title);
    }
    
    for(int i = pilha->n; i > 0; i--){
        printf("[%d] ", i-1);
        imprimir(pegar(pilha, i-1));
    }
	
	delPilhaLinear(pilha);
	delPilhaLinear(pilhaRemovidos);
	return 0;
}
