...
//+-+-+--+-+-+-+ Algoritmo shellSort +-+-+--+-+-+-+
// Função de comparação para o Shell Sort
int compareShows(const Show *a, const Show *b) {
    int cmp = strcmp(a->type, b->type);
    if (cmp == 0) { // Se os types forem iguais, compara por title
...
//+-+-+--+-+-+-+ Algoritmo QuickSort +-+-+--+-+-+-+
// Função para comparar datas (retorna negativo se a < b, 0 se iguais, positivo se a > b)
int compararDatas(struct tm a, struct tm b) {
...
int compareShowsQuick(const Show *a, const Show *b) {
    int cmp = compararDatas(a->date_added, b->date_added);
    if (cmp == 0) { // Se as datas forem iguais, compara por title
        return strcmp(a->title, b->title);
    }
    return cmp;
}
...
int particiona(Show *array, int esq, int dir) {
    // Escolhe o pivô (usando o elemento do meio)
    ...
    while (1) {
        do {
            i++;
            comp++;
        } while (compareShowsQuick(&array[i], &pivo) < 0);
        
        do {
            j--;
            comp++;
        } while (compareShowsQuick(&array[j], &pivo) > 0);
...
void quicksortRec(Show *array, int esq, int dir) {
    if (esq < dir) {
        int p = particiona(array, esq, dir);
        quicksortRec(array, esq, p);
...
//+-+-+--+-+-+-+ Algoritmo Bolha(date_added) +-+-+--+-+-+-+
void Bolha(Show *array, int n) {
    int i, j;
    for (i = 0; i < n-1; i++) {
        for (j = 0; j < n-i-1; j++) {
           
            int cmp_data = compararDatas(array[j].date_added, array[j+1].date_added);
            
            // Se as datas forem iguais, compara os títulos
            if (cmp_data == 0) {
                comp++;
                if (strcmp(array[j].title, array[j+1].title) > 0) {
                    swapShows(&array[j], &array[j+1]);
                }
            } 
...
int main() {
    ...
    Insercao(lista, tamanhoLista);
    clock_gettime(CLOCK_MONOTONIC, &fim);
    
    //For para printar os dados da lista
    for(int i=0;i<10;i++){printShow(lista, lista[i].show_id);} //Teste de ordenação
