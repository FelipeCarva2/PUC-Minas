...
// ---------------------------------------------------------------------------------------------------- //
//Variaveis globais
//#define FILE_PATH "/home/felipe/PUCMinas/AEDs/AEDs2/tp02/Q01/disneyplus.csv"
...
void arquivoLog(int matricula, double duracao) {
	FILE *arq = fopen("1543536_avl.txt", "w");
	if(arq) {
		fprintf(arq, "%d\t%d\t%fs", matricula, comp, duracao);
	}
	fclose(arq);
}

//---------------------------------------------------------------------------------------------------- //
    //Metodos No / ArvoreAVL
//---------------------------------------------------------------------------------------------------- //
...
int pesquisarRescursivo(No *i, char x[]){
    if(i == NULL){
        return -1;
    }else{
        comp++;
...
int pesquisar(ArvoreAvl *arvore, char x[]){
    printf("raiz");
    int resp = pesquisarRescursivo(arvore->raiz, x);
